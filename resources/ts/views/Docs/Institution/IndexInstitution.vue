...
const callbacks: IndexCrudCallbacks<App.Models.DocumentsInstitution> = {
    ...
    mapDeleteDetails: (instance, data) => {
        ...
        return instance.reactives.rows.filter((obj: any) => instance.reactives.selected.includes(obj.id)).map((obj: any) => {
            return {
                ...
                name: obj.name,
                code: obj.code,
                reset_sn_period: obj.reset_sn_period,
                sn_template: obj.sn_template,
            }
        })
    }
}
...
const config: IndexCrudConfig<any> = {
    ...
    columns: [
        ...
        {
            label: "Reset SN Perioid",
            field: "reset_sn_period",
            sortable: true,
        },
        {
            label: "SN template",
...
function perioidToLabel(code: string) {
    switch (code) {
        case "m":
            return "Monthly";
        case "y":
